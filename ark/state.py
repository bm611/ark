...
class State(rx.State):
        ...
        for file_ref in self.uploaded_files:
            if file_ref.get("type") == "image" and file_ref.get("presigned_url"):
                ...
                try:
                    ...
                    if presigned_url:
                        pdf_base64 = download_and_encode_pdf(
                            presigned_url,
                            file_ref.get("original_filename", "document.pdf"),
                        )
                        if pdf_base64:
                            content.append(
                                {
                    ...
                    print(
                        f"Error processing R2 PDF {file_ref.get('original_filename')}: {e}"
                    )
    ...
    def _get_model_for_action(self) -> str:
        ...
        elif self.selected_action == "Think":
            if self.selected_provider == ModelConfig.DEFAULT_PROVIDER:
                return ModelConfig.THINK_MODEL
            # Use selected model for other providers
            return self.selected_model if self.selected_model else None
        else:
    ...
    def handle_think_click(self):
        """Handle think button click - toggle action and set/reset model accordingly"""
        if self.selected_action == "Think":
            # Deactivating think - reset to defaults
            self.selected_action = ""
            self.selected_provider = ModelConfig.DEFAULT_PROVIDER
            self.selected_model = ModelConfig.CHAT_MODEL
        else:
            # Activating think - set think model
            self.selected_action = "Think"
            self.selected_provider = ModelConfig.DEFAULT_PROVIDER
            self.selected_model = ModelConfig.THINK_MODEL
                    ...
                    try:
                        ...
                        if r2_metadata:
                            ...
                            try:
                                ...
                                print(
                                    f"Could not remove local file {file.name}: {cleanup_e}"
                                )
    ...
    def clear_images(self):
        ...
        self.uploaded_files = [
            f for f in self.uploaded_files if f.get("type") != "image"
        ]
        ...
        if await chat_exists(chat_id, clerk_state.user_id):
                ...
                for file_data in chat_files:
                    try:
                        ...
                        if presigned_url:  # Only add files with valid presigned URLs
                            ...
                            print(
                                f"✓ Loaded file: {file_data['original_filename']} ({file_type}) with URL: {presigned_url[:50]}..."
                            )
                        else:
                            print(
                                f"✗ Failed to generate presigned URL for file: {file_data['original_filename']}"
                            )
                    except Exception as e:
                        print(
                            f"✗ Error processing file {file_data.get('original_filename', 'unknown')}: {e}"
                        )
                        continue
            ...
            for i, msg in enumerate(db_messages):
                if (
                    ...
                    and any(
                        item.get("type") in ["image_url", "file"]
            ...
            for i, msg in enumerate(db_messages):
                ...
                if isinstance(msg["content"], list):
                    if msg["role"] == "assistant":
                        ...
                        image_files = [
                            f for f in file_references if f.get("type") == "image"
                        ]
                        pdf_files = [
                            f for f in file_references if f.get("type") == "pdf"
                        ]
                        ...
                        for item in msg["content"]:
                            if isinstance(item, dict):
                                if item.get("type") == "image_url" and image_files:
                                    ...
                                    item["image_url"]["url"] = image_files[0][
                                        "presigned_url"
                                    ]
                                    print(
                                        f"Updated image URL with fresh presigned URL: {image_files[0]['presigned_url'][:50]}..."
                                    )
                                elif item.get("type") == "file" and pdf_files:
                                    # Update PDF file data if needed (though it's usually base64)
                                    print(
                                        f"Found PDF file reference: {pdf_files[0]['original_filename']}"
                                    )
                else:
                    content = msg["content"]
                ...
                if (
                    msg["role"] == "user"
                    and file_references
                    and i == message_with_files_index
                ):
                    chat_message["files"] = file_references
                    print(f"Added {len(file_references)} files to user message {i}")
                    for file_ref in file_references:
                        print(
                            f"  - {file_ref.get('original_filename')}: {file_ref.get('presigned_url')[:50]}..."
                            if file_ref.get("presigned_url")
                            else f"  - {file_ref.get('original_filename')}: NO URL"
                        )
